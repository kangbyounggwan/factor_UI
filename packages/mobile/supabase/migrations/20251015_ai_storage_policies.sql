...
-- 기존 정책 삭제 (있을 경우)
DROP POLICY IF EXISTS "Users can view their own ai-models files" ON storage.objects;
DROP POLICY IF EXISTS "Users can upload to their own ai-models folder" ON storage.objects;
DROP POLICY IF EXISTS "Users can update their own ai-models files" ON storage.objects;
DROP POLICY IF EXISTS "Users can delete their own ai-models files" ON storage.objects;
...
USING (
  bucket_id = 'ai-models'
  AND (SELECT (string_to_array(name, '/'))[1]) = auth.uid()::text
);
