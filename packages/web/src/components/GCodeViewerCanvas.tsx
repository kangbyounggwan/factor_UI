...
export const GCodeViewerCanvas = ({
  ...
  const parseGCode = useCallback(async (content: string) => {
    ...
    try {
      ...
      toast({
        title: 'G-code 파싱 완료',
        description: `${parsedModel.modelInfo.layerCount}개 레이어, ${parsedModel.modelInfo.totalFilament.toFixed(2)}mm 필라멘트`,
      });
    } catch (error) {
      console.error('[GCodeViewerCanvas] 파싱 에러:', error);
      toast({
        title: '파싱 실패',
        description: error instanceof Error ? error.message : 'G-code 파싱 중 오류 발생',
        variant: 'destructive',
      });
    } finally {
      setParsing(false);
    ...
    if (gcodeContent) {
      ...
      fetch(gcodeUrl)
        ...
        .catch((error) => {
          ...
          toast({
            title: '로드 실패',
            description: 'G-code 파일을 불러올 수 없습니다',
            variant: 'destructive',
          });
        });
    }
  }, [gcodeContent, gcodeUrl, parseGCode, toast]);
        ...
        {parsing && (
          <div className="absolute top-4 left-4 right-4 z-20 space-y-2 bg-background/95 backdrop-blur p-3 rounded-lg border">
            <div className="flex items-center justify-between text-sm">
              <span>{parseProgress.type === 'parsing' ? '파싱 중...' : '분석 중...'}</span>
