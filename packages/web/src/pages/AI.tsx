...
const AI = () => {
  ...
  const confirmPrinterSelection = async () => {
    ...
    try {
      ...
      const modelPrompt = currentModel?.prompt;  // 사용자의 원본 프롬프트
  ...
  const startPrint = async () => {
    ...
    try {
      // 0. 사용자가 입력한 printFileName을 우선 사용
      console.log('[AI][FILENAME] ========================================');
      console.log('[AI][FILENAME] printFileName (raw):', printFileName);

      // 사용자 입력값을 sanitize (한글, 영문, 숫자, 하이픈, 언더스코어 허용)
      const sanitizedInput = printFileName.trim().replace(/[^a-zA-Z0-9가-힣\-_]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '');
      console.log('[AI][FILENAME] sanitizedInput:', sanitizedInput);

      let fileName: string;

      // 사용자 입력값이 유효하면 그것을 사용
      if (sanitizedInput && sanitizedInput.length > 0) {
        fileName = sanitizedInput.endsWith('.gcode') ? sanitizedInput : `${sanitizedInput}.gcode`;
        console.log('[AI][FILENAME] Using user input:', fileName);
      } else {
        // 사용자 입력이 없으면 DB에서 short_filename 가져오기
        if (currentModelId) {
          ...
          console.log('[AI][FILENAME] DB query result:', { gcodeFile, gcodeError });

          if (gcodeFile?.short_filename && !gcodeError && gcodeFile.short_filename !== 'unnamed.gcode') {
            fileName = gcodeFile.short_filename;
            console.log('[AI][FILENAME] Using short_filename from DB:', fileName);
          } else {
            // DB에도 없으면 타임스탬프 기반 파일명 생성
            fileName = `print_${Date.now()}.gcode`;
            console.log('[AI][FILENAME] Using timestamp fallback:', fileName);
          }
        } else {
          fileName = `print_${Date.now()}.gcode`;
          console.log('[AI][FILENAME] No modelId, using timestamp fallback:', fileName);
        }
      }

      console.log('[AI][FILENAME] FINAL fileName to send:', fileName);
      console.log('[AI][FILENAME] ========================================');
      ...
      const shouldNavigate = window.confirm(
        t('ai.navigateToDashboardConfirm', { printerName: selectedPrinter.name }) ||
        `${selectedPrinter.name} 대시보드로 이동하시겠습니까?`
      );
      if (shouldNavigate) {
