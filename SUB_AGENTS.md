...
### 10. **dev-finalize** - 개발 마무리 파이프라인 ⭐
- **역할**: 개발 완료 후 품질 검증 및 정리 자동화
- **트리거**: `@dev-finalize` 또는 `개발 마무리`
- **4단계 파이프라인**:
  1. **Quality Check** - 수정된 파일 품질 검사 (TypeScript, ESLint, 빌드)
  2. **Migration Cleanup** - 중복 마이그레이션 정리, packages/shared 통합
  3. **MD Cleanup** - 완료된 PLAN 문서 삭제, 중복 문서 정리
  4. **Docs Update** - 수정된 페이지 기준 문서 업데이트
- **참고**: [dev-finalize.md](.claude/agents/dev-finalize.md)
...
- **[.claude/agents/dev-finalize.md](./.claude/agents/dev-finalize.md)** - 개발 마무리 파이프라인 ⭐
